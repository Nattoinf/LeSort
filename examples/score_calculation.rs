...
fn main() {
    ...
    // Calculate and display scores for each scenario
    for (name, file_count, type_count) in scenarios {
        ...
        // Remove emoji for cleaner table alignment
        let interpretation_clean = interpretation
            .replace("✅", "")
            .replace("👍", "")
            .replace("⚠️", "")
            .replace("❌", "")
        ...
        println!("{:<25} {:>10} {:>5} {:>9.2}% {}", 
                 name, file_count, type_count, score, interpretation_clean);
    }
